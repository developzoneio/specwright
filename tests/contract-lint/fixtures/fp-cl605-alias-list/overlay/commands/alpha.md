...
Model references are aliases only (`sonnet`, `haiku`, `opus`, `inherit`). The command never
escalates the agent's tool allowlist, and a draft that is not large keeps the default model.
